import logging

import os
from pathlib import Path

import requests
import tensorflow as tf
from Configs.Config import Config
from Data.DataUtilities import download

class ConfigNeuralStyleTransfer(Config):

    config = {
        "settings": {
            "use_wandb": True,
            "logging_level": logging.INFO,
            "printing_epoch_interval": 100,
        },

        "input_data": {
            "content_image_path": download(file_path="Data/NeuralStyleTransfer/content.jpg", origin="https://hips.hearstapps.com/hmg-prod.s3.amazonaws.com/images/dog-puppy-on-garden-royalty-free-image-1586966191.jpg?crop=1.00xw:0.669xh;0,0.190xh&resize=1200:*"),
            "style_image_path": download(file_path="Data/NeuralStyleTransfer/style.jpg", origin="https://camo.githubusercontent."),
            "input_image_width": None,
            "input_image_height": None,
            "input_image_channels": None

        },
        "output_data": {
            "output_path": "Logs/NeuralStyleTransfer/",
            "output_prefix": "doggo_generated",
            "generated_image_resize_factor": None,
            "generated_image_width": 400,
            "generated_image_height": None
        },

        "hyperparameters": {
            "style_layer_names": [
                "block1_conv1",
                "block2_conv1",
                "block3_conv1",
                "block4_conv1",
                "block5_conv1",
            ],
            "content_layer_name": "block4_conv2",
            "total_variation_weight": 1e-4,
            "style_weight": 1e-4,
            "content_weight": 1e-7,
            "learning_rate": 10.0,

            "iterations": 5000,
        }
    }
    def __init__(self):
        super().__init__()

    def setup(self):
        self.config["input_data"]["input_image_width"] = tf.keras.preprocessing.image.load_img(self.config["input_data"]["content_image_path"]).size[0]
        self.config["input_data"]["input_image_height"] = tf.keras.preprocessing.image.load_img(self.config["input_data"]["content_image_path"]).size[1]
        self.config["input_data"]["input_image_channels"] = 3 if tf.keras.preprocessing.image.load_img(self.config["input_data"]["content_image_path"]).mode == "RGB" else 1

        self.config["output_data"]["generated_image_width"] = int(self.config["input_data"]["input_image_width"] * self.config["output_data"]["generated_image_resize_factor"]) if self.config["output_data"]["generated_image_resize_factor"] else self.config["output_data"]["generated_image_width"]
        self.config["output_data"]["generated_image_height"] = int(self.config["input_data"]["input_image_width"] * self.config["output_data"]["generated_image_width"] / self.config["input_data"]["input_image_height"])

        return self.config



